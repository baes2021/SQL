...
where weight like 1.5;

-- 7.2 What is the total weight of all the packages that he sent?
select sum(weight),sender
from package p
join client c
on c.accountnumber=p.sender
group by sender
having sender = (select c.accountnumber
from client c
join package p
on c.accountnumber=p.recipient
where weight like 1.5) 
...
FROM Client 
WHERE Client.Name LIKE 'Al Gore%')

SELECT SUM(Weight) AS 'Total package weight'
FROM Package
JOIN Al_Gore
ON Package.Sender = Al_Gore.Account;
